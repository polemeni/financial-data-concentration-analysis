...
import type { TimeConcentrationAnalysisResponse } from '../types';
    ...
    <div className="step-container">
      ...
      {Object.entries(results.concentration_data).map(
                    ...
                    {results.concentration_buckets.map(bucket => {
                      const { valueKey, countKey } =
                        getBucketProperties(bucket);
                                             return (
                         <tr key={bucket}>
                           <td>Top {bucket}% ({periods[0]?.[countKey] || 0})</td>
                           {results.time_periods.map(period => {
       ...
       <div className="results-summary">
         ...
         <p><strong>Time Columns:</strong> {results.time_columns.join(', ')}</p>
         <p><strong>Aggregate Columns:</strong> {results.aggregate_columns.join(', ')}</p>
         <p><strong>Concentration Buckets:</strong> {results.concentration_buckets.map(b => `${b}%`).join(', ')}</p>
       </div>
